...
/*
    ...
    function exhibition_setting($param1 = null, $param2 = null, $param3 = null){

        if ($param1 == 'insert'){
            $this->crud_model->insert_exhibition();
        ...
        if($param1 == 'update'){
            $this->crud_model->update_exhibition($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data successfully updated'));
            redirect(base_url(). 'superadmin/exhibition_setting', 'refresh');
        }

        if($param1 == 'delete'){
            $this->crud_model->delete_exhibition($param2);
            $this->session->set_flashdata('flash_message', get_phrase('Data successfully deleted'));
            redirect(base_url(). 'superadmin/exhibition_setting', 'refresh');
        }

        $page_data['page_name']         = 'exhibition_setting';
        $page_data['page_title']        = get_phrase('Manage exhibition');

        $exhibition_id = $this->session->userdata('exhibition_id');
        $page_data['select_exhibition']   = $this->db->get_where('exhibition')->result_array();
